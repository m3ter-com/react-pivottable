...
describe('  utils', function() {
    describe('.PivotData()', function() {
        ...
        describe('with array-of-array input', function() {
            ...
            it('has the correct grand total value', () =>
                expect(pd.getAggregator([], []).value()).toBe(
                    (1 + 3) / (2 + 4)
                ));
        });
        ...
        describe('with rows/cols', function() {
            ...
            it('returns matching records', function() {
                const records = [];
                pd.forEachMatchingRecord({ gender: 'male' }, x =>
                    records.push(x.name)
                );
    ...
    describe('.aggregatorTemplates', function() {
        ...
        describe('.countUnique', () =>
            it('works', () =>
                expect(getVal(tpl.countUnique(), ['gender'])).toBe(2)));

        describe('.listUnique', () =>
            it('works', () =>
                expect(getVal(tpl.listUnique(), ['gender'])).toBe(
                    'male,female'
                )));

        describe('.average', () =>
            it('works', () =>
                expect(getVal(tpl.average(), ['trials'])).toBe(103)));
        ...
        describe('.first', () =>
            it('works', () =>
                expect(getVal(tpl.first(), ['name'])).toBe('Carol')));

        describe('.last', () =>
            it('works', () =>
                expect(getVal(tpl.last(), ['name'])).toBe('Nick')));
        ...
        describe('.median', () =>
            it('works', () =>
                expect(getVal(tpl.median(), ['trials'])).toBe(102.5)));
        ...
        describe('.var', () =>
            it('works', () =>
                expect(getVal(tpl.var(), ['trials'])).toBe(
                    48.666666666666686
                )));

        describe('.stdev', () =>
            it('works', () =>
                expect(getVal(tpl.stdev(), ['trials'])).toBe(
                    6.976149845485451
                )));
    ...
    describe('.sortAs()', function() {
        ...
        it('sorts with unknown values sorted at the end', () =>
            expect([5, 2, 3, 4, 1].sort(sortAs([4, 3, 2]))).toEqual([
                4,
                3,
                2,
                1,
    ...
    describe('.derivers', function() {
        ...
        describe('.bin()', function() {
            ...
            it('bins negative numbers', () =>
                expect(binner({ x: -12 })).toBe(-10));

            it("doesn't bin strings", () =>
                expect(binner({ x: 'a' })).toBeNaN());

            it("doesn't bin objects", () =>
                expect(binner({ x: { a: 1 } })).toBeNaN());
        });
    });
});
